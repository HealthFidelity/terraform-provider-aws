...
func testSweepCloudformationStackSetInstances(region string) error {
	...
	for _, stackSet := range stackSets {
		...
		for _, instance := range instances {
			...
			if err := tfcloudformation.WaitStackSetOperationSucceeded(conn, stackSetName, aws.StringValue(output.OperationId), tfcloudformation.StackSetInstanceDeletedDefaultTimeout); err != nil {
